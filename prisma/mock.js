// =========================
// Styles (13개)
// =========================
export const Styles = [
  {
    id: 1,
    thumbnail: 'https://picsum.photos/id/11/600/400_tn',
    nickname: 'keony',
    title: '미니멀 오피스룩',
    content: '화이트 셔츠와 블랙 슬랙스로 구성된 깔끔한 출근룩.',
    password: 'pwpwpwpw1',
    viewCount: 1200,
    curationCount: 1,
    tags: ['미니멀', '오피스룩', '모노톤'],
    imageUrls: ['https://picsum.photos/id/11/600/400'],
    createdAt: '2025-01-01T00:00:00Z',
    updatedAt: '2025-01-02T00:00:00Z'
  },
  {
    id: 2,
    thumbnail: 'https://picsum.photos/id/12/600/400_tn',
    nickname: 'sora',
    title: '봄 데일리 캐주얼',
    content: '가벼운 자켓과 청바지로 완성한 캐주얼 봄 코디.',
    password: 'pwpwpwpw2',
    viewCount: 980,
    curationCount: 1,
    tags: ['간절기', '캐쥬얼', '봄패션'],
    imageUrls: ['https://picsum.photos/id/12/600/400'],
    createdAt: '2025-02-02T00:00:00Z',
    updatedAt: '2025-02-03T00:00:00Z'
  },
  {
    id: 3,
    thumbnail: 'https://picsum.photos/id/21/600/400_tn',
    nickname: 'minsu',
    title: '겨울 감성룩',
    content: '코트와 머플러로 따뜻한 분위기의 감성 스타일.',
    password: 'pwpwpwpw3',
    viewCount: 1500,
    curationCount: 2,
    tags: ['겨울룩', '감성'],
    imageUrls: ['https://picsum.photos/id/21/600/400'],
    createdAt: '2025-03-03T00:00:00Z',
    updatedAt: '2025-03-04T00:00:00Z'
  },
  {
    id: 4,
    thumbnail: 'https://picsum.photos/id/41/600/400_tn',
    nickname: 'yuna',
    title: '여름 스트릿룩',
    content: '와이드 팬츠와 크롭티로 시원한 스트릿 무드 완성.',
    password: 'pwpwpwpw4',
    viewCount: 800,
    curationCount: 1,
    tags: ['스트릿', '여름패션', '캐쥬얼'],
    imageUrls: ['https://picsum.photos/id/41/600/400'],
    createdAt: '2025-04-04T00:00:00Z',
    updatedAt: '2025-04-05T00:00:00Z'
  },
  {
    id: 5,
    thumbnail: 'https://picsum.photos/id/51/600/400_tn',
    nickname: 'taeyang',
    title: '가을 데이트룩',
    content: '브라운 가디건과 데님으로 따뜻한 데이트 감성.',
    password: 'pwpwpwpw5',
    viewCount: 1100,
    curationCount: 3,
    tags: ['데이트룩', '감성', '캐쥬얼'],
    imageUrls: ['https://picsum.photos/id/51/600/400', 'https://picsum.photos/id/52/600/400'],
    createdAt: '2025-05-05T00:00:00Z',
    updatedAt: '2025-05-06T00:00:00Z'
  },
  {
    id: 6,
    thumbnail: 'https://images.unsplash.com/photo-1682685794760-3e7e1d796be4_tn',
    nickname: 'minji',
    title: '가을 캐주얼 데일리룩',
    content: '부드러운 가디건과 데님 팬츠로 완성한 따뜻한 데일리룩.',
    password: 'mnJ482skQ9',
    viewCount: 132,
    curationCount: 1,
    tags: ['가을룩', '캐쥬얼', '데님'],
    imageUrls: ['https://images.unsplash.com/photo-1682685794760-3e7e1d796be4'],
    createdAt: '2025-06-06T10:00:00Z',
    updatedAt: '2025-06-07T09:30:00Z'
  },
  {
    id: 7,
    thumbnail: 'https://picsum.photos/id/51/600/400_tn',
    nickname: 'soobin',
    title: '오피스 미니멀룩',
    content: '화이트 셔츠와 블랙 슬랙스로 심플한 오피스룩.',
    password: 'sbnA93lpXf1',
    viewCount: 208,
    curationCount: 1,
    tags: ['미니멀룩', '모노톤', '오피스룩'],
    imageUrls: ['https://picsum.photos/id/51/600/400'],
    createdAt: '2025-07-07T11:00:00Z',
    updatedAt: '2025-07-08T08:45:00Z'
  },
  {
    id: 8,
    thumbnail: 'https://picsum.photos/id/52/600/400_tn',
    nickname: 'yuna',
    title: '편안한 주말 스트릿룩',
    content: '후드티와 조거팬츠로 완성한 캐주얼 스트릿 스타일.',
    password: 'yuN549zRt8',
    viewCount: 97,
    curationCount: 1,
    tags: ['스트릿룩', '캐쥬얼', '트레이닝'],
    imageUrls: ['https://picsum.photos/id/52/600/400'],
    createdAt: '2025-08-08T09:00:00Z',
    updatedAt: '2025-08-09T10:10:00Z'
  },
  {
    id: 9,
    thumbnail: 'https://images.unsplash.com/photo-1682685794760-3e7e1d796be4_tn',
    nickname: 'hajin',
    title: '간절기 레이어드룩',
    content: '셔츠와 니트를 함께 매치해 포근하면서 세련된 간절기 코디.',
    password: 'hjL92sQp87zA',
    viewCount: 185,
    curationCount: 1,
    tags: ['레이어드룩', '간절기'],
    imageUrls: ['https://images.unsplash.com/photo-1682685794760-3e7e1d796be4'],
    createdAt: '2025-09-09T10:00:00Z',
    updatedAt: '2025-09-10T11:00:00Z'
  },
  {
    id: 10,
    thumbnail: 'https://images.unsplash.com/photo-1679932156433-2f909f1b01d7_tn',
    nickname: 'eunho',
    title: '겨울 오버핏 코트룩',
    content: '롱코트와 머플러로 완성한 감성적인 겨울 스타일.',
    password: 'ehX84nLp22tZ',
    viewCount: 266,
    curationCount: 1,
    tags: ['겨울룩', '감성', '오버핏'],
    imageUrls: ['https://images.unsplash.com/photo-1679932156433-2f909f1b01d7'],
    createdAt: '2025-10-10T09:00:00Z',
    updatedAt: '2025-10-11T10:15:00Z'
  },
  {
    id: 11,
    thumbnail: 'https://images.unsplash.com/photo-1680951732398-7a7ff76d43f0_tn',
    nickname: 'sora',
    title: '여름 린넨 셋업룩',
    content: '린넨 셔츠와 팬츠로 시원하게 연출한 썸머 셋업 스타일.',
    password: 'soR56nMv03yB',
    viewCount: 119,
    curationCount: 1,
    tags: ['린넨', '여름룩'],
    imageUrls: ['https://images.unsplash.com/photo-1680951732398-7a7ff76d43f0'],
    createdAt: '2025-11-11T08:30:00Z',
    updatedAt: '2025-11-12T09:45:00Z'
  },
  {
    id: 12,
    thumbnail: 'https://images.unsplash.com/photo-1684324872744-9948e1e272db_tn',
    nickname: 'jimin',
    title: '하프집 맨투맨 캐주얼룩',
    content: '트레이닝 팬츠와 하프집 맨투맨으로 완성한 가벼운 캐주얼룩.',
    password: 'jmF39pKs71vR',
    viewCount: 152,
    curationCount: 1,
    tags: ['트레이닝', '캐쥬얼', '맨투맨'],
    imageUrls: ['https://images.unsplash.com/photo-1684324872744-9948e1e272db'],
    createdAt: '2025-12-12T10:00:00Z',
    updatedAt: '2025-12-13T11:20:00Z'
  },
  {
    id: 13,
    thumbnail: 'https://images.unsplash.com/photo-1679932156433-2f909f1b01d7_tn',
    nickname: 'hyunwoo',
    title: '블랙앤화이트 모노톤룩',
    content: '깔끔한 대비감으로 포멀한 인상을 주는 모노톤 스타일.',
    password: 'hwT67qNj94bP',
    viewCount: 230,
    curationCount: 1,
    tags: ['모노톤', '오피스룩', '포멀'],
    imageUrls: ['https://images.unsplash.com/photo-1679932156433-2f909f1b01d7'],
    createdAt: '2025-12-30T09:00:00Z',
    updatedAt: '2025-12-31T10:40:00Z'
  }
];

// =========================
// Categories (22개)
// =========================
export const Categories = [
  {
    id: 1,
    type: 'TOP',
    name: '화이트 셔츠',
    brand: 'Uniqlo',
    price: 39000,
    styleId: 1,
    createdAt: '2025-01-01T00:00:00Z',
    updatedAt: '2025-01-01T00:00:00Z'
  },
  {
    id: 2,
    type: 'BOTTOM',
    name: '블랙 슬랙스',
    brand: 'Musinsa',
    price: 49000,
    styleId: 1,
    createdAt: '2025-01-01T00:00:00Z',
    updatedAt: '2025-01-01T00:00:00Z'
  },
  {
    id: 3,
    type: 'SHOES',
    name: '로퍼',
    brand: 'Dr.Martens',
    price: 129000,
    styleId: 1,
    createdAt: '2025-01-01T00:00:00Z',
    updatedAt: '2025-01-01T00:00:00Z'
  },
  {
    id: 4,
    type: 'TOP',
    name: '데님 자켓',
    brand: 'Zara',
    price: 69000,
    styleId: 2,
    createdAt: '2025-01-01T00:00:00Z',
    updatedAt: '2025-01-01T00:00:00Z'
  },
  {
    id: 5,
    type: 'BOTTOM',
    name: '라이트진',
    brand: 'Levi’s',
    price: 89000,
    styleId: 2,
    createdAt: '2025-01-01T00:00:00Z',
    updatedAt: '2025-01-01T00:00:00Z'
  },
  {
    id: 6,
    type: 'OUTER',
    name: '울 코트',
    brand: 'TNGT',
    price: 189000,
    styleId: 3,
    createdAt: '2025-01-01T00:00:00Z',
    updatedAt: '2025-01-01T00:00:00Z'
  },
  {
    id: 7,
    type: 'ACCESSORY',
    name: '머플러',
    brand: 'H&M',
    price: 29000,
    styleId: 3,
    createdAt: '2025-01-01T00:00:00Z',
    updatedAt: '2025-01-01T00:00:00Z'
  },
  {
    id: 8,
    type: 'TOP',
    name: '크롭티',
    brand: '8Seconds',
    price: 19000,
    styleId: 4,
    createdAt: '2025-01-01T00:00:00Z',
    updatedAt: '2025-01-01T00:00:00Z'
  },
  {
    id: 9,
    type: 'BOTTOM',
    name: '와이드팬츠',
    brand: 'Nike',
    price: 59000,
    styleId: 4,
    createdAt: '2025-01-01T00:00:00Z',
    updatedAt: '2025-01-01T00:00:00Z'
  },
  {
    id: 10,
    type: 'OUTER',
    name: '가디건',
    brand: 'Musinsa',
    price: 49000,
    styleId: 5,
    createdAt: '2025-01-01T00:00:00Z',
    updatedAt: '2025-01-01T00:00:00Z'
  },
  {
    id: 11,
    type: 'BOTTOM',
    name: '데님팬츠',
    brand: 'Zara',
    price: 69000,
    styleId: 5,
    createdAt: '2025-01-01T00:00:00Z',
    updatedAt: '2025-01-01T00:00:00Z'
  },
  {
    id: 12,
    type: 'TOP',
    name: '니트 가디건',
    brand: 'Musinsa Standard',
    price: 49000,
    styleId: 6,
    createdAt: '2025-10-01T10:20:00Z',
    updatedAt: '2025-10-03T09:50:00Z'
  },
  {
    id: 13,
    type: 'BOTTOM',
    name: '데님 팬츠',
    brand: "Levi's",
    price: 89000,
    styleId: 6,
    createdAt: '2025-10-01T10:21:00Z',
    updatedAt: '2025-10-03T09:51:00Z'
  },
  {
    id: 14,
    type: 'TOP',
    name: '화이트 셔츠',
    brand: 'Musinsa Standard',
    price: 29900,
    styleId: 7,
    createdAt: '2025-10-02T11:25:00Z',
    updatedAt: '2025-10-04T09:00:00Z'
  },
  {
    id: 15,
    type: 'BOTTOM',
    name: '블랙 슬랙스',
    brand: 'Musinsa Standard',
    price: 49900,
    styleId: 7,
    createdAt: '2025-10-02T11:26:00Z',
    updatedAt: '2025-10-04T09:01:00Z'
  },
  {
    id: 16,
    type: 'TOP',
    name: '후드티',
    brand: 'Musinsa Standard',
    price: 39000,
    styleId: 8,
    createdAt: '2025-10-05T09:15:00Z',
    updatedAt: '2025-10-06T10:30:00Z'
  },
  {
    id: 17,
    type: 'BOTTOM',
    name: '조거팬츠',
    brand: 'Musinsa Standard',
    price: 45000,
    styleId: 8,
    createdAt: '2025-10-05T09:16:00Z',
    updatedAt: '2025-10-06T10:31:00Z'
  },
  {
    id: 18,
    type: 'TOP',
    name: '브이넥 니트',
    brand: 'Musinsa Standard',
    price: 39000,
    styleId: 9,
    createdAt: '2025-10-07T10:30:00Z',
    updatedAt: '2025-10-07T11:20:00Z'
  },
  {
    id: 19,
    type: 'OUTER',
    name: '울 블렌드 코트',
    brand: 'Musinsa Standard',
    price: 129000,
    styleId: 10,
    createdAt: '2025-10-08T09:25:00Z',
    updatedAt: '2025-10-08T10:30:00Z'
  },
  {
    id: 20,
    type: 'TOP',
    name: '린넨 셔츠',
    brand: 'Musinsa Standard',
    price: 34900,
    styleId: 11,
    createdAt: '2025-10-09T08:55:00Z',
    updatedAt: '2025-10-09T10:00:00Z'
  },
  {
    id: 21,
    type: 'TOP',
    name: '하프집 맨투맨',
    brand: 'Musinsa Standard',
    price: 35900,
    styleId: 12,
    createdAt: '2025-10-10T10:30:00Z',
    updatedAt: '2025-10-10T11:35:00Z'
  },
  {
    id: 22,
    type: 'BOTTOM',
    name: '블랙 슬림 팬츠',
    brand: 'Musinsa Standard',
    price: 49000,
    styleId: 13,
    createdAt: '2025-10-11T09:25:00Z',
    updatedAt: '2025-10-11T11:00:00Z'
  }
];

// =========================
// Curations (16개)
// =========================
export const Curations = [
  {
    id: 1,
    nickname: 'curator_jay',
    password: 'curJay11',
    content: '화이트 셔츠와 슬랙스의 밸런스가 훌륭해요. 출근룩으로 추천!',
    trendy: 8,
    personality: 6,
    practicality: 9,
    costEffectiveness: 7,
    stylesId: 1,
    createdAt: '2025-10-01T11:00:00Z',
    updatedAt: '2025-10-01T11:00:00Z'
  },
  {
    id: 2,
    nickname: 'curator_ella',
    password: 'curElla22',
    content: '봄 분위기에 어울리는 라이트진이 포인트네요.',
    trendy: 7,
    personality: 8,
    practicality: 8,
    costEffectiveness: 6,
    stylesId: 2,
    createdAt: '2025-10-02T11:00:00Z',
    updatedAt: '2025-10-02T11:00:00Z'
  },
  {
    id: 3,
    nickname: 'curator_hyun',
    password: 'curHyun33',
    content: '코트핏이 깔끔하고 전체적으로 감성적인 무드예요.',
    trendy: 9,
    personality: 7,
    practicality: 6,
    costEffectiveness: 5,
    stylesId: 3,
    createdAt: '2025-10-03T09:10:00Z',
    updatedAt: '2025-10-03T09:10:00Z'
  },
  {
    id: 4,
    nickname: 'curator_jisoo',
    password: 'curJisoo44',
    content: '스트릿 감성이 확실히 살아있어요! 여름에도 무겁지 않네요.',
    trendy: 10,
    personality: 9,
    practicality: 6,
    costEffectiveness: 7,
    stylesId: 4,
    createdAt: '2025-10-03T10:00:00Z',
    updatedAt: '2025-10-03T10:00:00Z'
  },
  {
    id: 5,
    nickname: 'curator_min',
    password: 'curMin55',
    content: '브라운톤 코디가 가을 감성과 잘 어울립니다.',
    trendy: 6,
    personality: 7,
    practicality: 8,
    costEffectiveness: 8,
    stylesId: 5,
    createdAt: '2025-10-04T10:00:00Z',
    updatedAt: '2025-10-04T10:00:00Z'
  },
  {
    id: 6,
    nickname: 'curator_aria',
    password: 'curAri66',
    content: '가디건 컬러가 너무 따뜻해요! 전체적인 톤이 부드럽네요.',
    trendy: 7,
    personality: 8,
    practicality: 9,
    costEffectiveness: 7,
    stylesId: 6,
    createdAt: '2025-10-05T09:00:00Z',
    updatedAt: '2025-10-05T09:00:00Z'
  },
  {
    id: 7,
    nickname: 'curator_june',
    password: 'curJune77',
    content: '화이트 셔츠+블랙 슬랙스 조합은 실패할 수 없죠.',
    trendy: 5,
    personality: 5,
    practicality: 10,
    costEffectiveness: 9,
    stylesId: 7,
    createdAt: '2025-10-05T09:20:00Z',
    updatedAt: '2025-10-05T09:20:00Z'
  },
  {
    id: 8,
    nickname: 'curator_sue',
    password: 'curSue88',
    content: '편하게 입을 수 있어서 주말용으로 딱이네요.',
    trendy: 6,
    personality: 7,
    practicality: 10,
    costEffectiveness: 8,
    stylesId: 8,
    createdAt: '2025-10-06T10:00:00Z',
    updatedAt: '2025-10-06T10:00:00Z'
  },
  {
    id: 9,
    nickname: 'curator_ryu',
    password: 'curRyu99',
    content: '니트랑 셔츠 조합이 예쁘고 따뜻해 보여요.',
    trendy: 8,
    personality: 7,
    practicality: 9,
    costEffectiveness: 8,
    stylesId: 9,
    createdAt: '2025-10-07T10:10:00Z',
    updatedAt: '2025-10-07T10:10:00Z'
  },
  {
    id: 10,
    nickname: 'curator_nam',
    password: 'curNam10',
    content: '코트 핏이 완벽합니다. 겨울 오피스룩으로 강추.',
    trendy: 9,
    personality: 6,
    practicality: 7,
    costEffectiveness: 6,
    stylesId: 10,
    createdAt: '2025-10-08T09:30:00Z',
    updatedAt: '2025-10-08T09:30:00Z'
  },
  {
    id: 11,
    nickname: 'curator_luna',
    password: 'curLuna11',
    content: '린넨 소재가 자연스럽고 여름에 잘 어울려요.',
    trendy: 8,
    personality: 8,
    practicality: 9,
    costEffectiveness: 9,
    stylesId: 11,
    createdAt: '2025-10-09T08:50:00Z',
    updatedAt: '2025-10-09T08:50:00Z'
  },
  {
    id: 12,
    nickname: 'curator_dan',
    password: 'curDan12',
    content: '하프집 맨투맨 코디가 정말 편해 보여요!',
    trendy: 7,
    personality: 6,
    practicality: 9,
    costEffectiveness: 8,
    stylesId: 12,
    createdAt: '2025-10-10T10:00:00Z',
    updatedAt: '2025-10-10T10:00:00Z'
  },
  {
    id: 13,
    nickname: 'curator_hana',
    password: 'curHana13',
    content: '모노톤 스타일 좋아하는 사람한테 완전 추천합니다.',
    trendy: 9,
    personality: 7,
    practicality: 8,
    costEffectiveness: 7,
    stylesId: 13,
    createdAt: '2025-10-11T09:00:00Z',
    updatedAt: '2025-10-11T09:00:00Z'
  },
  {
    id: 14,
    nickname: 'curator_soo',
    password: 'curSoo14',
    content: '색감 조화가 부드럽고 안정적이에요.',
    trendy: 6,
    personality: 6,
    practicality: 8,
    costEffectiveness: 9,
    stylesId: 5,
    createdAt: '2025-10-12T10:00:00Z',
    updatedAt: '2025-10-12T10:00:00Z'
  },
  {
    id: 15,
    nickname: 'curator_jake',
    password: 'curJake15',
    content: '전체적으로 균형 잡힌 스타일링이에요.',
    trendy: 8,
    personality: 7,
    practicality: 9,
    costEffectiveness: 8,
    stylesId: 3,
    createdAt: '2025-10-13T10:00:00Z',
    updatedAt: '2025-10-13T10:00:00Z'
  },
  {
    id: 16,
    nickname: 'curator_sooyoung',
    password: 'sooyoungbabo',
    content: '평상복으로도 좋지만 캐쥬얼 오피스룩으로도 적당할 것 같아요.',
    trendy: 7,
    personality: 5,
    practicality: 10,
    costEffectiveness: 8,
    stylesId: 5,
    createdAt: '2025-11-13T10:00:00Z',
    updatedAt: '2025-11-13T10:00:00Z'
  }
];

// =========================
// Comments (16개)
// =========================
export const Comments = [
  {
    id: 1,
    content: '저도 이렇게 입고 출근했는데 반응 좋았어요!',
    password: 'pwpwpwpw1',
    curationId: 1,
    createdAt: '2025-10-01T12:00:00Z',
    updatedAt: '2025-10-01T12:00:00Z'
  },
  {
    id: 2,
    content: '라이트진 코디가 진짜 봄 느낌이에요.',
    password: 'pwpwpwpw2',
    curationId: 2,
    createdAt: '2025-10-02T12:00:00Z',
    updatedAt: '2025-10-02T12:00:00Z'
  },
  {
    id: 3,
    content: '이 코트 어디 브랜드인가요? 핏이 예뻐요.',
    password: 'pwpwpwpw3',
    curationId: 3,
    createdAt: '2025-10-03T09:30:00Z',
    updatedAt: '2025-10-03T09:30:00Z'
  },
  {
    id: 4,
    content: '여름에 입기 딱 좋아 보여요. 시원해 보입니다.',
    password: 'pwpwpwpw4',
    curationId: 4,
    createdAt: '2025-10-03T10:10:00Z',
    updatedAt: '2025-10-03T10:10:00Z'
  },
  {
    id: 5,
    content: '가을 느낌 물씬 나네요 🍂',
    password: 'pwpwpwpw5',
    curationId: 5,
    createdAt: '2025-10-04T10:30:00Z',
    updatedAt: '2025-10-04T10:30:00Z'
  },
  {
    id: 6,
    content: '이 가디건 어디 건지 궁금해요!',
    password: 'mnJ482skQ9',
    curationId: 6,
    createdAt: '2025-10-05T09:20:00Z',
    updatedAt: '2025-10-05T09:20:00Z'
  },
  {
    id: 7,
    content: '심플한 오피스룩 최고죠 😎',
    password: 'sbnA93lpXf1',
    curationId: 7,
    createdAt: '2025-10-05T09:40:00Z',
    updatedAt: '2025-10-05T09:40:00Z'
  },
  {
    id: 8,
    content: '편해보이고 꾸안꾸 느낌이에요!',
    password: 'yuN549zRt8',
    curationId: 8,
    createdAt: '2025-10-06T10:20:00Z',
    updatedAt: '2025-10-06T10:20:00Z'
  },
  {
    id: 9,
    content: '레이어드 조합 참고할게요 👕',
    password: 'hjL92sQp87zA',
    curationId: 9,
    createdAt: '2025-10-07T10:30:00Z',
    updatedAt: '2025-10-07T10:30:00Z'
  },
  {
    id: 10,
    content: '이 코트 진짜 고급스럽네요.',
    password: 'ehX84nLp22tZ',
    curationId: 10,
    createdAt: '2025-10-08T09:40:00Z',
    updatedAt: '2025-10-08T09:40:00Z'
  },
  {
    id: 11,
    content: '린넨 셋업 꼭 사야겠어요.',
    password: 'soR56nMv03yB',
    curationId: 11,
    createdAt: '2025-10-09T09:10:00Z',
    updatedAt: '2025-10-09T09:10:00Z'
  },
  {
    id: 12,
    content: '하프집 맨투맨 조합 완전 내 스타일이에요.',
    password: 'jmF39pKs71vR',
    curationId: 12,
    createdAt: '2025-10-10T10:30:00Z',
    updatedAt: '2025-10-10T10:30:00Z'
  },
  {
    id: 13,
    content: '모노톤 스타일 너무 세련됐어요.',
    password: 'hwT67qNj94bP',
    curationId: 13,
    createdAt: '2025-10-11T09:20:00Z',
    updatedAt: '2025-10-11T09:20:00Z'
  },
  {
    id: 14,
    content: '톤온톤 조합이 안정적이네요.',
    password: 'pwpwpwpw5',
    curationId: 14,
    createdAt: '2025-10-12T10:20:00Z',
    updatedAt: '2025-10-12T10:20:00Z'
  },
  {
    id: 15,
    content: '이 스타일 밸런스 너무 좋네요 👌',
    password: 'pwpwpwpw3',
    curationId: 15,
    createdAt: '2025-10-13T10:20:00Z',
    updatedAt: '2025-10-13T10:20:00Z'
  },
  {
    id: 16,
    content: '두루두루 입기 편한 스타일이죠 👌',
    password: 'pwpwpwpw5',
    curationId: 16,
    createdAt: '2025-10-13T10:20:00Z',
    updatedAt: '2025-10-13T10:20:00Z'
  }
];
